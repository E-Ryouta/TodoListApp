...
@bp.route("/container_order", methods=["PUT"])
def put_container_order():
    tasks = to_snake_case(request.json)
    if not tasks:
        return jsonify({"message": "Tasks not found"}), 400

    update_tasks = todo_list_service.update_container_order(tasks)
